...
const manifest = {
    ...
    description: "Stream Tamil and Tamil Dubbed movies directly in Stremio. Scraped on-the-fly from Moviesda.",
...
builder.defineCatalogHandler(async (args) => {
    ...
    console.log(`[Addon] Catalog request - type: ${type}, id: ${id}, extra:`, extra);

    if (type === 'movie' && id === 'moviesda-search' && extra && extra.search) {
        try {
            console.log(`[Addon] Performing catalog search for: "${extra.search}"`);
            ...
            console.log(`[Addon] Returned ${metas.length} search results to Stremio`);
            return { metas };
        } catch (err) {
            console.error('[Addon] Catalog handler error:', err);
            return { metas: [] };
        }
    }
...
builder.defineStreamHandler(async (args) => {
    const { type, id } = args;
    console.log(`[Addon] Stream request - type: ${type}, id: ${id}`);
    ...
    try {
        ...
        if (id.startsWith('moviesda_tmdb:')) {
            tmdbId = id.split(':')[1];
            console.log(`[Addon] Processing custom catalog TMDB ID: ${tmdbId}`);
        } else if (id.startsWith('tt')) {
            // Global Stremio catalog request using IMDb ID (tt...)
            console.log(`[Addon] Processing global IMDb ID ${id}. Querying TMDB Find API...`);
            ...
            if (findData && findData.movie_results && findData.movie_results.length > 0) {
                ...
                console.log(`[Addon] Successfully mapped IMDb ${id} ➔ TMDB ID ${tmdbId}`);
            } else {
                console.warn(`[Addon] No TMDB mapping found for IMDb ID ${id}`);
            }
        }

        if (!tmdbId) {
            console.warn(`[Addon] No valid TMDB ID resolved. Cannot find streams.`);
            return { streams: [] };
        }

        // Step 2: Resolve Moviesda Page URL & Metadata
        console.log(`[Addon] Resolving Moviesda page URL for TMDB ID: ${tmdbId}`);
        const resolved = await resolveMovieByTmdbId(tmdbId);
        
        if (!resolved || !resolved.url) {
            console.warn(`[Addon] Movie not found on Moviesda index for TMDB ID ${tmdbId}`);
            return { streams: [] };
        }

        const moviePageUrl = resolved.url;
        const movieTitle = resolved.metadata.title;
        console.log(`[Addon] Found page URL on Moviesda: ${moviePageUrl} (Title: ${movieTitle})`);

        // Step 3: Scrape Stream URLs from the page
        const allStreams = await scrapeMovieStreams(moviePageUrl, movieTitle);
        console.log(`[Addon] Scraped ${allStreams.length} raw streams from Moviesda`);
        ...
        const filteredStreams = allStreams.filter(stream => {
            ...
            if (stream.size !== "Unknown Size" && sizeMB < 50) {
                console.log(`[Addon] Excluding small stream: "${stream.title}" (${stream.size})`);
                return false;
            }
            return true;
        });
        ...
        console.log(`[Addon] Returning ${stremioStreams.length} sorted, playable streams to Stremio`);
        return { streams: stremioStreams };

    } catch (err) {
        console.error('[Addon] Stream handler error:', err);
        return { streams: [] };
    }
});
...
console.log(`\n======================================================`);
console.log(`🎬 Moviesda Stremio Addon is running at:`);
